...
ENV_VAR_PREFIX=`echo $REDIS_PREFIX|awk '{print toupper($0)}'|sed 's/-/_/g'`
...
function launchmaster() {
  ...
  kubectl -n $NAMESPACE label --overwrite pod $HOSTNAME redis-role="master"
...
function launchsentinel() {
  ...
  kubectl -n $NAMESPACE label --overwrite pod $HOSTNAME redis-role="sentinel"
  ...
  while true; do
    ...
    MASTER_IP=$(kubectl -n $NAMESPACE get pod -o jsonpath='{range .items[*]}{.metadata.name} {..podIP} {.status.containerStatuses[0].state}{"\n"}{end}' -l redis-role=master|grep running|grep $REDIS_PREFIX|awk '{print $2}'|xargs)
...
function launchslave() {
  kubectl -n $NAMESPACE label --overwrite pod $HOSTNAME redis-role="slave"
...
MASTERS=`kubectl -n $NAMESPACE get pod -o jsonpath='{range .items[*]}{.metadata.name} {..podIP} {.status.containerStatuses[0].state}{"\n"}{end}' -l redis-role=master|grep running|grep $REDIS_PREFIX`
if [[ "$MASTERS" == "" ]]; then
  echo "No masters found: \"$MASTERS\" Electing first master..."
  SLAVE1=`kubectl -n $NAMESPACE get pod -o jsonpath='{range .items[*]}{.metadata.creationTimestamp} {.metadata.name} {.status.containerStatuses[0].state} {"\n"} {end}' -l redis-node=true |grep running|sort|awk '{print $2}'|grep $REDIS_PREFIX|head -n1`
